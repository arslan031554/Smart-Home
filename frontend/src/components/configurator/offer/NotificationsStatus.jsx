...
export default function NotificationsStatus({ emailSent, smsSent }) {
    const notifications = [
        { id: 'email', name: 'Email Confirmation', icon: Mail, type: 'Electronic Specification Copy', sent: emailSent },
        { id: 'sms', name: 'Mobile Link', icon: MessageSquare, type: 'Portable Documentation Access', sent: smsSent },
    ];

    return (
        <div className="space-y-6">
            <h3 className="text-sm font-bold text-slate-500 uppercase tracking-widest flex items-center gap-3 ml-2">
                <Activity className="w-5 h-5 text-primary-500" /> Digital Transmission Status
            </h3>
                ...
                {notifications.map((notif) => (
                    <Card key={notif.id} className="p-8 h-full border border-slate-100 shadow-sm flex flex-col justify-between group hover:shadow-premium transition-all duration-300 rounded-[2.5rem] bg-white">
                        ...
                        <div className="flex items-center justify-between pt-6 border-t border-slate-50">
                            <span className="text-[10px] font-bold text-slate-300 uppercase tracking-widest italic">Transmission Service</span>
                            {notif.sent ? (
                                <div className="flex items-center gap-2 px-4 py-1.5 bg-emerald-50 rounded-xl border border-white shadow-sm">
                                    <CheckCircle2 className="w-4 h-4 text-emerald-500" />
                                    <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest leading-none">Dispatched</span>
                                </div>
                            ) : (
                                <div className="flex items-center gap-2 px-4 py-1.5 bg-amber-50 rounded-xl border border-white shadow-sm">
                                    <Activity className="w-4 h-4 text-amber-500 animate-pulse" />
                                    <span className="text-[10px] font-black text-amber-600 uppercase tracking-widest leading-none">Queued</span>
                                </div>
                            )}
                        </div>
                    </Card>
