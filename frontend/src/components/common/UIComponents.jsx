...
import {
    X, AlertTriangle, CheckCircle2, Info, AlertCircle, Loader2, Check,
} from 'lucide-react';
...
export const AnimatedCard = ({ children, className, hover = true, delay = 0, ...props }) => (
    <div
        className={cn(
            'premium-card rounded-[1.75rem] border border-surface-border text-textPrimary transition-all duration-300 animate-slide-up',
        ...
        <div className="relative z-10 flex items-start justify-between gap-4">
            <div className="space-y-4">
                ...
                <div className="space-y-2">
                    <span className="block font-heading text-4xl font-semibold leading-none text-textPrimary">{value}</span>
                    {trend != null && (
            ...
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-primary-500/20 bg-primary-500/10 text-primary-400 shadow-soft">
...
export const SectionTitle = ({ title, subtitle, badge, align = 'left', className }) => (
    <div className={cn('space-y-4 mb-8 animate-fade-in', align === 'center' && 'text-center', className)}>
        ...
        <h2 className="text-4xl md:text-5xl font-heading font-semibold text-textPrimary tracking-tight leading-none">{title}</h2>
        {subtitle ? (
...
export const PremiumTableWrapper = ({ children, className }) => (
    <div className={cn('premium-card overflow-hidden rounded-[1.75rem] border border-surface-border', className)}>
...
export const Button = React.forwardRef(({
    ...
    const variants = {
        ...
        secondary: 'bg-[#ffffff] text-primary-700 border border-primary-700 shadow-soft hover:bg-primary-50 hover:border-primary-800',
        accent: 'bg-primary-500 text-primary-950 shadow-glow hover:bg-primary-400',
        outline: 'bg-transparent text-primary-700 border border-primary-700/45 hover:bg-primary-50 hover:border-primary-700',
    ...
    const sizes = {
        ...
        lg: 'h-14 px-8 text-sm md:text-base rounded-full',
    };
...
export const Input = React.forwardRef(({ label, error, icon: Icon, className, ...props }, ref) => (
    <div className="w-full space-y-2">
        ...
        <div className="group relative flex items-center">
            {Icon ? <Icon className="pointer-events-none absolute left-4 h-4.5 w-4.5 text-textSecondary transition-colors group-focus-within:text-primary-400" /> : null}
            <input
                ref={ref}
                className={cn(
                    'w-full rounded-2xl border border-white/10 bg-[#1f1f1f] py-3.5 text-sm font-medium text-textPrimary shadow-inner transition-all duration-300',
                    'placeholder:text-textSecondary focus:border-primary-500/50 focus:outline-none focus:ring-4 focus:ring-primary-500/10',
...
export const Badge = ({ children, variant = 'info', className }) => {
    const variants = {
        info: 'bg-primary-500/12 text-primary-700 border-primary-500/20',
        primary: 'bg-primary-500/14 text-primary-700 border-primary-500/25',
    ...
    return (
        <span className={cn(
            'inline-flex items-center rounded-full border px-3.5 py-1.5 text-[10px] font-semibold uppercase tracking-[0.24em]',
            variants[variant],
            className,
        )}>
            {children}
    ...
    <div className="flex w-fit flex-col items-start gap-1.5">
        <label className={cn('inline-flex cursor-pointer items-center gap-3', className)}>
            <div className="relative flex-shrink-0">
                ...
                <div className="h-5 w-5 rounded-md border border-white/16 bg-[#1f1f1f] transition-all duration-200 peer-checked:border-primary-500 peer-checked:bg-primary-500/20" />
                <Check className="absolute inset-0 h-5 w-5 scale-0 text-primary-300 transition-transform duration-200 peer-checked:scale-90" />
            </div>
...
export const Switch = React.forwardRef(({ label, className, ...props }, ref) => (
    <label className={cn('inline-flex cursor-pointer items-center gap-4', className)}>
        <div className="relative flex-shrink-0">
            ...
            <div className="h-6 w-12 rounded-full border border-white/12 bg-white/8 transition-colors duration-300 peer-checked:border-primary-500/30 peer-checked:bg-primary-500/25" />
            <div className="absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-textPrimary transition-transform duration-300 peer-checked:translate-x-6" />
        </div>
...
export const Tooltip = ({ children, content }) => (
    <div className="group relative inline-block">
        {children}
        <div className="pointer-events-none absolute bottom-full left-1/2 z-50 mb-3 -translate-x-1/2 translate-y-2 whitespace-nowrap rounded-xl border border-white/10 dark-surface px-4 py-2 text-[11px] font-semibold text-textPrimary opacity-0 shadow-premium transition-all duration-200 group-hover:translate-y-0 group-hover:opacity-100">
            {content}
...
export const Avatar = ({ src, name, size = 'md', className }) => {
    ...
    return (
        <div className={cn(
            'flex items-center justify-center overflow-hidden rounded-full border border-primary-500/18 bg-primary-500/12 font-semibold text-primary-200 shadow-soft',
            sizes[size],
            className,
        )}>
            {src
...
export const Skeleton = ({ className, repeat = 1 }) => (
    <>
        {Array.from({ length: repeat }).map((_, i) => (
            <motion.div
                key={i}
                className={cn('rounded-xl bg-white/8', className)}
...
export const EmptyState = ({ title, description, icon: Icon, action }) => (
    ...
    >
        <div className="flex h-24 w-24 items-center justify-center rounded-[2rem] border border-white/10 bg-white/5 text-primary-400 shadow-soft">
