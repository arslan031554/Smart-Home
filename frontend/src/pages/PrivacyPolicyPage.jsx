...
export default function PrivacyPolicyPage() {
    ...
    const paragraphs = [
        t('legal.privacy.paragraphs.0'),
        t('legal.privacy.paragraphs.1'),
        t('legal.privacy.paragraphs.2'),
        t('legal.privacy.paragraphs.3'),
        t('legal.privacy.paragraphs.4'),
    ];
        ...
        <AnimatedPageWrapper className="mx-auto max-w-4xl space-y-8 pb-16">
            ...
            <Card className="rounded-[2rem] p-8">
                <div className="mb-5 flex items-center gap-3">
                    ...
                    <h3 className="text-lg font-medium text-textPrimary">{t('footer.privacyPolicy')}</h3>
                </div>

                <div className="space-y-5 text-sm leading-relaxed text-textSecondary">
                    {paragraphs.map((paragraph, index) => (
                        <p key={`privacy-paragraph-${index + 1}`}>{paragraph}</p>
                    ))}
                </div>
            </Card>
        </AnimatedPageWrapper>
