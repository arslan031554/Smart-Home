...
function Counter({ stat }) {
  ...
  useEffect(() => {
    if (!started || stat.textValue) return;
    let frame = 0;
    const frames = 72;
    const tick = () => {
      frame += 1;
  ...
  return (
    <div ref={ref} className="group relative overflow-hidden rounded-lg border border-white/10 bg-white/[0.08] p-7 text-center transition hover:-translate-y-1 hover:border-orange/40 hover:bg-white/12">
      <div className="absolute inset-0 rounded-lg bg-emerald/5 opacity-0 transition group-hover:opacity-100" />
      <div className="relative">
        <div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-full border border-emerald/20 bg-emerald/10">
          <Icon className="text-emerald" size={28} />
        </div>
        <div className="text-5xl font-black text-white sm:text-6xl">
          {stat.textValue || count}
          {!stat.textValue && <span className="text-orange">{stat.suffix}</span>}
        </div>
...
export default function Stats() {
  const { stats, statsSection } = usePresentationContent();
    ...
    <AnimatedSection className="relative overflow-hidden bg-ink py-20">
      ...
      <div className="section-shell container-px relative">
        <div className="mb-12 text-center">
          <span className="section-kicker">{statsSection.kicker}</span>
          <h2 className="mt-4 text-3xl font-black uppercase text-white sm:text-4xl">{statsSection.title}</h2>
        </div>
