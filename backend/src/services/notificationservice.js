...
export const sendOtpSms = async (phone, otpCode) => {
    const body = `Your Smart Building Configurator code: ${otpCode}. Valid for 10 mins.`;
...
export const sendTestEmail = async ({ to, subject, text, html } = {}) => {
    ...
    const finalSubject = String(subject || '').trim() || 'Smart Building Configurator SendGrid Test';
    ...
    const finalHtml = String(html || '').trim() || `
        <div style="font-family: sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
            ...
            <p>This test email was generated by the Smart Building Configurator backend.</p>
